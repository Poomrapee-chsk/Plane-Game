...
public:
  ...
  glm::vec3 GetRandomStartLocation() {
    // Get random spawn location on map (X, Y) as (x, _, z)
    float x = -width + ((float)rand() / RAND_MAX * (2 * width));
    float z = -height + ((float)rand() / RAND_MAX * (2 * height));

    // Sample the height from the coordinates
...
struct Plane {
  ...
  void Reset() {
    Position = glm::vec3(Terrain->GetRandomStartLocation());
    Pitch = 0.0f;
    Roll = 0.0f;
    Yaw = 0.0f;
    Speed = 20.0f;
...
int main() {
  ...
  glm::mat4 fontProjection = glm::ortho(0.0f, static_cast<float>(SCR_WIDTH),
                                        0.0f, static_cast<float>(SCR_HEIGHT));
  fontShader.use();
  fontShader.setInt("text", 0);
  glUniformMatrix4fv(glGetUniformLocation(fontShader.ID, "projection"), 1,
  ...
  std::string font_name = FileSystem::getPath("resources/fonts/modern_dos.ttf");
  ...
  /* Enable blending to render fonts */
  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

  /* Reserve buffer for fonts */
